...
{
	char var[128] = "TERM";
	static char str[128] = "TERM=putty";
	...
	if (n != 2) {
		plog_info("\tUnable to extract ncurses version from %s\n", ncurses_version);
	} else {
		plog_info("\tncurses version = %d.%d (%s)\n", max_ver, min_ver, ncurses_version);
	}

	old_value = getenv(var);
	if ((max_ver >= 6) && (min_ver >= 1) && (strcmp(old_value, "xterm") == 0)) {
		...
		plog_info("\tChanged TERM from %s to %s\n", old_value, new_value);
	} else {
