...
void set_EUI(struct ipv6_addr *ipv6_addr, prox_rte_ether_addr *mac)
{
	...
	ipv6_addr->bytes[8] = ipv6_addr->bytes[8] ^ (ipv6_addr->bytes[8] & 0x02); 	// Invert Universal/local bit
...
void create_mac_from_EUI(struct ipv6_addr *ipv6_addr, prox_rte_ether_addr *mac)
{
	...
	mac->addr_bytes[0] = mac->addr_bytes[0] ^ (mac->addr_bytes[0] & 0x02);
...
void build_neighbour_advertisement(struct task_base *tbase, struct rte_mbuf *mbuf, prox_rte_ether_addr *target)
{
	...
	if (memcmp(peth->s_addr.addr_bytes, &null_addr, sizeof(struct ipv6_addr)) == 0) {
		memcpy(peth->d_addr.addr_bytes, &prox_cfg.all_nodes_mac_addr, sizeof(struct ipv6_addr));
		memcpy(ipv6_hdr->dst_addr, &prox_cfg.all_nodes_ipv6_mcast_addr, sizeof(struct ipv6_addr));
	} else {
		memcpy(peth->d_addr.addr_bytes, peth->s_addr.addr_bytes, sizeof(struct ipv6_addr));
	...
	ipv6_hdr->vtc_flow = 0x00000060;
	ipv6_hdr->payload_len = rte_cpu_to_be_16(sizeof(struct icmpv6_NA) + sizeof(struct icmpv6_option));
	ipv6_hdr->proto = ICMPv6;
	ipv6_hdr->hop_limits = 255;
	memcpy(ipv6_hdr->src_addr, &task->internal_port_table[port_id].ipv6_addr, sizeof(struct ipv6_addr));
	...
	// neighbour_advertisement->destination_address is already set to neighbour_sollicitation->target_address
	neighbour_advertisement->options.type = ICMPv6_source_link_layer_address;
	neighbour_advertisement->options.length = 1;	// 8 bytes
	memcpy(&neighbour_advertisement->options.data, &task->internal_port_table[port_id].mac, sizeof(prox_rte_ether_addr));

	struct icmpv6_option *option = (&neighbour_advertisement->options + 1);
