...
void task_start_l3(struct task_base *tbase, struct task_args *targ)
{
	...
	struct prox_port_cfg *port = find_reachable_port(targ);
		...
		if (targ->flags & TASK_ARG_NDP) {
			...
			plog_info("\tCore %d, task %d, IPv6 addr is "IPv6_BYTES_FMT" (%s)\n", targ->lconf->id, targ->id, IPv6_BYTES(targ->local_ipv6.bytes), IP6_Canonical(&targ->local_ipv6));
			memcpy(&tbase->l3.local_ipv6, &targ->local_ipv6, sizeof(struct ipv6_addr));

			if (targ->ipv6_router)
				register_router_to_ctrl_plane(tbase->l3.tmaster, tbase->l3.reachable_port_id, targ->lconf->id, targ->id, &targ->local_ipv6, &targ->router_prefix);
			else
				register_node_to_ctrl_plane(tbase->l3.tmaster, &targ->local_ipv6, tbase->l3.reachable_port_id, targ->lconf->id, targ->id);
		}
