...
static int task_gen_add_range(struct task_base *tbase, struct task_args *targ)
{
	struct task_gen *task = (struct task_gen *)tbase;
	...
	for (uint32_t i = 0; i < targ->n_ranges; ++i) {
		task->ranges[task->n_ranges].min = targ->range[i].min;
		task->ranges[task->n_ranges].value = targ->range[i].min;
		uint32_t m = targ->range[i].max;
		task->ranges[task->n_ranges].range_len = 0;
		while (m != 0) {
    			m >>= 8;
    			task->ranges[task->n_ranges].range_len++;
		}
		task->ranges[task->n_ranges].offset = targ->range[i].offset;
		task->ranges[task->n_ranges++].max = targ->range[i].max;
	}
	return 0;
}
...
static void init_task_gen(struct task_base *tbase, struct task_args *targ)
{
	...
	PROX_PANIC(task_gen_add_range(tbase, targ), "Failed to add range\n");
}
