...
static inline void build_value_ipv6(struct task_gen *task, uint32_t mask, int var_bit_pos, int init_var_bit_pos, struct ipv6_addr val, struct ipv6_addr fixed_bits)
{
	...
	if (var_bit_pos < 32) {
		...
		register_node_to_ctrl_plane(tbase->l3.tmaster, &val, tbase->l3.reachable_port_id, tbase->l3.core_id, tbase->l3.task_id);
	}
}
	...
	for (uint32_t i = 0; i < task->n_pkts; ++i) {
		...
		if ((ether_type == ETYPE_MPLSU) || (ether_type == ETYPE_MPLSM)) {
			...
			if (ip->version_ihl >> 4 == 4)
				ipv4 = 1;
			else if (ip->version_ihl >> 4 != 6)	// Version field at same location for IPv6 and IPv6
				continue;
		...
		if (ipv4) {
			...
			register_node_to_ctrl_plane(tbase->l3.tmaster, ip6_src, tbase->l3.reachable_port_id, tbase->l3.core_id, tbase->l3.task_id);
