...
void init_rte_dev(int use_dummy_devices)
{
	...
	for (uint8_t port_id = 0; port_id < PROX_MAX_PORTS; ++port_id) {
		...
		if (port_cfg->vdev[0]) {
			...
			prox_port_cfg[vdev_port_id].netmask = prox_port_cfg[port_id].netmask;
...
static void init_port(struct prox_port_cfg *port_cfg)
{
	...
	if (prox_port_cfg[port_id].ip) {
		set_ip_address(prox_port_cfg[port_id].name, &prox_port_cfg[port_id].ip, prox_port_cfg[port_id].netmask);
	}
