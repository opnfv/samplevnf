...
Introduction
^^^^^^^^^^^^^^^
L2L3 stack comprises of a set of libraries which are coomonly used by all
...
It Provides wrapper APIâ€™s on top of DPDKâ€™s LAG(link Aggregation) APIâ€™s, This 
...
application.In this document, ACL will be explained in detail with its own building
blocks.

This document assumes the reader possesses the knowledge of DPDK concepts and packet
framework. For more details, read DPDK Getting Started Guide, DPDK Programmers Guide,
...
----------
The DPDK IP Pipeline Framework provides a set of libraries to build a pipeline
application. In this document, vPE will be explained in detail with its own
building blocks.
